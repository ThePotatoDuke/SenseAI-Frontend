...
class FakeMultipartRequest extends Fake
    implements http.MultipartRequest {} // i also tried it with this

void main() {
  setUpAll(() {
    registerFallbackValue(
        FakeBaseRequest()); // Register fallback for BaseRequest
  });
  ...
  group('sendText', () {
    test('returns a reply if the text sending http call completes successfully',
        () async {
    ...
    test(
        'throws an exception if the http call completes with an error when sending text',
        () async {
  ...
  group("send image list", () {
    test("returns a reply if images are successfully sent", () async {
      ...
      final responseStream =
          Stream.value(utf8.encode('{"reply": "This is a response"}'));
      ...
      // Mock file behaviors
      when(() => mockFile1.openRead())
          .thenAnswer((_) => Stream.value([])); // Use thenAnswer for Stream
      when(() => mockFile2.openRead())
          .thenAnswer((_) => Stream.value([])); // Use thenAnswer for Stream
    ...
    test("throws an exception if the server returns an error", () async {
      ...
      try {
        ...
        fail(
            'Exception was not thrown'); // If no exception is thrown, fail the test
      } catch (e) {
